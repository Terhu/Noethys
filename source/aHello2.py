...
class Hyperlien_regroupement(Hyperlink.HyperLinkCtrl):
    ...
    def OnLeftLink(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            # MAJ du listView
            self.parent.olv_piecesFiltrees.SetColonneTri(indexChoix+1)
...
class Hyperlien_periodes(Hyperlink.HyperLinkCtrl):
    ...
    def OnLeftLink(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            # MAJ
            self.parent.olv_piecesFiltrees.SetListePeriodes(listePeriodes)
##            self.parent.MAJtotal()
            self.parent.grid_sizer_options.Layout()
            self.parent.Refresh()
        dlg.Destroy()
...
class Panel(wx.Panel):
    ...
    def __init__(self, parent, IDpayeur=None):
        ...
        # OL Prestations
        #self.listviewAvecFooter = OL_Prestations.ListviewAvecFooter(self, kwargs={"IDpayeur" : IDpayeur})
        self.listviewAvecFooter = aOL_FacturationPieces.ListviewAvecFooter(self, None, kwargs={"IDpayeur" : IDpayeur})
...
##
##        self. = wx.StaticText(self, -1, _(u"Total = 0.00 %s") % SYMBOLE)
##        self..Show(False)
...
##        grid_sizer_options.Add(self., 0, wx.ALL, 0)
...
##        grid_sizer_prestations.Add(grid_sizer_options, 1, wx.EXPAND|wx.ALL, 0)
    ...
    def OnBoutonAjouter(self, event):
        self.olv_piecesFiltrees.Ajouter(None)

    def OnBoutonModifier(self, event):
        self.olv_piecesFiltrees.Modifier(None)
    ...
    def OnBoutonImprimer(self, event):
        if len(self.olv_piecesFiltrees.Selection()) == 0:
            noSelection = True
        else:
            noSelection = False
            ID = self.olv_piecesFiltrees.Selection()[0].IDprestation
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.ctrl= Panel(panel, IDpayeur=6163)
        self.ctrl.MAJ()
