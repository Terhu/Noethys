...
# -*- coding: iso-8859-15 -*-
#------------------------------------------------------------------------
# Application :    Noethys, gestion multi-activités, modifié pour supprimer les pièces avec
...
import aGestionDB as GestionDB
                ...
                if len(retour) >0:
                    if retour[0][0]!= None:
                        ...
                        if track.IDactivite == 0:
                            ...
                            if ret == False: ret = fGest.GetPieceModif999(self,track.IDcompte_payeur, None, IDnumPiece = track.IDcontrat )
                        if ret == None:
                            dictDonnees = fGest.dictPiece
                            suppressible = fGest.PieceSuppressible(self,dictDonnees)
                            if not suppressible :
                                GestionDB.MessageBox(self, u"Piece associée bloquée, ne peut pas être supprimée!\nElle est consultable et peut faire l'objet d'un avoir")
                                return
                            fGest.Suppression(self,dictDonnees,fGest.nbPieces)
                            self.MAJ()
...
class MyFrame(wx.Frame):
    def __init__(self, footer,*args, **kwds):
        ...
        if footer == "sans":
            ...
            self.listviewAvecFooter = ListviewAvecFooter(self, kwargs={"IDfamille" : 6163})
